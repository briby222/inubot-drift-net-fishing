...
public class PokeFishTask extends Task {
    ...
    public void notify(ChatMessageEvent event) {
        ...
        Integer target = Players.self().getTargetIndex();
    ...
    public boolean execute() {
        ...
        Npc nearest = fish.nearest();

        if (nearest.distance(Players.self()) == 1) { // dodesnt appear as targget if 1 tile away ? idfk
            pokedFish.put(nearest.getIndex(), Game.getTick());
        }
